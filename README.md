...
2. Fetch latest wine-staging from Kron4ek → `build/wine/`
3. Clone yabridge git master → `build/yabridge-src/`
4. Build with meson/ninja → `build/yabridge/`
5. Generate `env.sh`
6. Initialize an isolated WINEPREFIX at `prefix/`
...
rm -rf build/wine && ./setup.sh --no-yabridge

# Pin a specific wine version
./setup.sh --wine-version 11.7
...
Wrapper around `yabridge-test` that sources `env.sh` and activates the test
harness venv. Passes all arguments through.
